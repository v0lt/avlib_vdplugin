...
SET /A COUNT=0
FOR %%D IN (%FFMPEG_DLLS%) DO IF EXIST "ffmpeg\bin_x64\%%D" SET /A COUNT+=1
...
SET FFMPEG_ZIP=ffmpeg-n8.1-latest-win64-gpl-shared-8.1.zip

IF NOT EXIST ffmpeg\%FFMPEG_ZIP% (
  ECHO Downloading "%FFMPEG_ZIP%"...
  curl -o "ffmpeg\%FFMPEG_ZIP%" --insecure -L "https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/%FFMPEG_ZIP%"
)
...
"%SEVENZIP%" e "ffmpeg\%FFMPEG_ZIP%" -o"ffmpeg\bin_x64\" %FFMPEG_DLLS% -r -aos
"%SEVENZIP%" e "ffmpeg\%FFMPEG_ZIP%" -o"ffmpeg\lib_x64\" %FFMPEG_LIBS% -r -aos
"%SEVENZIP%" e "ffmpeg\%FFMPEG_ZIP%" -o"ffmpeg\lib_x64\" %FFMPEG_DEFS% -r -aos
