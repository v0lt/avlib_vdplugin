...
public:
	...
	void ChangeRateControl(CodecNVENC_HEVC::Config* config);
	INT_PTR DlgProc(UINT msg, WPARAM wParam, LPARAM lParam) override;
};

void ConfigNVENC_HEVC::ChangeRateControl(CodecNVENC_HEVC::Config* pConfig)
{
	if (pConfig->rc == CodecNVENC_HEVC::NVENC_HEVC_RC_VBR) {
...
INT_PTR ConfigNVENC_HEVC::DlgProc(UINT msg, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_HSCROLL:
		if ((HWND)lParam == GetDlgItem(mhdlg, IDC_ENC_RATECONTROL_SLIDER)) {
			...
			if (config->rc == CodecNVENC_HEVC::NVENC_HEVC_RC_VBR) {
...
bool CodecNVENC_HEVC::init_ctx(VDXPixmapLayout* layout)
{
	...
	if (codec_config.rc == NVENC_HEVC_RC_VBR) {
