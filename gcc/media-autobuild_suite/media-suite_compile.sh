...
if { { [[ $ffmpeg != "n" ]] && enabled libbluray; } || ! mpv_disabled libbluray; } &&
    ...
    sed -i 's;git://.*;https://git.videolan.org/git/libudfread.git;' .gitmodules
...
if [[ $ffmpeg != "n" ]]; then
    ...
    if do_vcs_skip "https://git.ffmpeg.org/ffmpeg.git"; then
        enabled libopenh264 &&
...
if [[ $xpcomp = "n" && $mpv != "n" ]] && pc_exists libavcodec libavformat libswscale libavfilter; then
    ...
    if ! mpv_disabled egl-angle; then
        ...
        if [[ $bits = 64bit && $angle = y ]]; then
            if do_vcs "https://chromium.googlesource.com/angle/angle#commit=7e1200b43cb6" angleproject; then
